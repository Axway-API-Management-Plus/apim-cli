...
public class JSONAPIManagerConfigAdapterTest extends APIManagerMockBase {
	...
	private static final String PACKAGE = "com/axway/apim/setup/config/adapter/";
	...
	public void testNonStagedConfig() throws AppException {
		...
		Assert.assertEquals(managerConfig.getPortalName(), "My API Manager");
		Assert.assertTrue(managerConfig.getGlobalFaultHandlerPolicy().getId().startsWith("<key"));
		Assert.assertEquals(managerConfig.getGlobalFaultHandlerPolicy().getName(), "Default Fault Handler");
	}
	...
	public void testValidStagedConfig() throws AppException {
		...
		Assert.assertEquals(managerConfig.getPortalName(), "Axway API Manager Test-Stage");
	}
