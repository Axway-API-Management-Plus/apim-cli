...
public class ConsoleRemoteHostsExporter extends RemoteHostsResultHandler {
	...
	public void export(List<RemoteHost> remoteHosts) throws AppException {
		...
		case standard:
			printStandard(remoteHosts);
			break;
		case wide:
			printWide(remoteHosts);
			break;
		case ultra:
			printUltra(remoteHosts);
		}
	}
	
	private void printStandard(List<RemoteHost> remoteHosts) {
	...
	private void printWide(List<RemoteHost> remoteHosts) {
		System.out.println(AsciiTable.getTable(borderStyle, remoteHosts, Arrays.asList(
				new Column().header("Id").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> remoteHost.getId()),
				new Column().header("Name").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> remoteHost.getName()),
				new Column().header("Port").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> Integer.toString(remoteHost.getPort())),
	...
	private void printUltra(List<RemoteHost> remoteHosts) {
		System.out.println(AsciiTable.getTable(borderStyle, remoteHosts, Arrays.asList(
				new Column().header("Id").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> remoteHost.getId()),
				new Column().header("Name").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> remoteHost.getName()),
				new Column().header("Port").headerAlign(HorizontalAlign.LEFT).dataAlign(HorizontalAlign.LEFT).with(remoteHost -> Integer.toString(remoteHost.getPort())),
	...
	private void printDetails(List<RemoteHost> remoteHosts) {
		...
		System.out.println(String.format("%-30s", "Created By: ") + remoteHost.getCreatedBy().getName());
